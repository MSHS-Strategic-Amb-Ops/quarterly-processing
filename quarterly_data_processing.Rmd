...
master_amb_mapping_prov <- as.data.frame(read_excel("/nfs/data/Applications/Ambulatory/master_amb_mapping_provider.xlsx", col_types = "text"))


# master_amb_mapping <- as.data.frame(read_excel(file.choose(), sheet = "Department"))
# master_amb_mapping_prov <- as.data.frame(read_excel(file.choose(), col_types = "text"))


# Mapping by Provider
master_amb_mapping_prov <- master_amb_mapping_prov %>%
  rename(Site = CAMPUS,
  ...
  dplyr::select(Site, DepartmentId, Campus.Specialty.Group, Campus.Specialty.SubGroup, Provider, NPI, Managed, `Office/Procedure`) %>%
  mutate(uniqueId = paste0(DepartmentId, Provider, NPI)) %>%
...
scheduling_data_raw$Managed <- master_amb_mapping_prov$Managed[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]
scheduling_data_raw <- scheduling_data_raw %>%
  filter(Managed == "Department")

## Office vs. Procedure Departments
scheduling_data_raw$Office <- master_amb_mapping_prov$`Office/Procedure`[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]

## Manual Mapping of Specialty and Site
scheduling_data_raw$Site <- master_amb_mapping_prov$Site[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]
scheduling_data_raw$Campus.Specialty.Group <- master_amb_mapping_prov$Campus.Specialty.Group[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]
scheduling_data_raw$Campus.Specialty.SubGroup <- master_amb_mapping_prov$Campus.Specialty.SubGroup[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]
...
referrals <- referrals %>%
   mutate(uniqueId = paste0(ReceivedByDepID, ReceivedByProvider, ReceivedByProvNPI)) 

referrals$Managed <-  master_amb_mapping_prov$Managed[match(referrals$uniqueId, master_amb_mapping_prov$uniqueId)]

referrals <- referrals %>%
  filter(Managed == "Department")

## Office vs. Procedure Departments
# referrals$Office <- referrals$`Office/Procedure`[match(scheduling_data_raw$uniqueId, master_amb_mapping_prov$uniqueId)]

## Manual Mapping of Specialty and Site
referrals$Campus <- master_amb_mapping_prov$Site[match(referrals$uniqueId, master_amb_mapping_prov$uniqueId)]
referrals$Campus.Specialty.Group <- master_amb_mapping_prov$Campus.Specialty.Group[match(referrals$uniqueId, master_amb_mapping_prov$uniqueId)]
referrals$Campus.Specialty.SubGroup <- master_amb_mapping_prov$Campus.Specialty.SubGroup[match(referrals$uniqueId, master_amb_mapping_prov$uniqueId)]
...
monthly_referral_vol <- referrals %>%
  group_by(Campus.Specialty.Group, Campus.Specialty.SubGroup, Campus, ReceivedByDept, StartInstant, ApptStatusFinal) %>%
  summarise(total = n()) %>%
  ...
  group_by(Campus, Campus.Specialty.Group, Campus.Specialty.SubGroup, Received.Year, Received.YearQtr, Received.Month, ApptStatusFinal) %>%
  summarise(total = n())
...
# # Merge with Rad Onc and Radiology Data
# monthly_vol <- bind_rows(monthly_vol, rad_radOnc_data)
monthly_vol_office <- monthly_vol %>%
  # dplyr::select(-Month) %>%
  pivot_wider(
    names_from = Visit.Method,
    values_from = monthly_vol,
...
## Arrived quarterly volume by Campus.Specialty, Appt.YearQtr, dept_zip_code, lat, lang, total_vol
vol_zip_code_office <- scheduling_data_raw %>%
  filter(Office == "Office") %>%
  filter(Appt.DateYear >= quarter_start & Appt.DateYear < quarter_end) %>%
  filter(!is.na(Campus)) %>%
  filter(Campus %in% c("NYEE","MSB","MSM","MSH-MSDFP","ONCOLOGY","MSW","MSBI","MSUS","MSH- AMBULATORY CARE")) %>%
...
vol_zip_code_office$dept_zip_code <- dept_zip_code_ref$`Address Zip Code`[match(vol_zip_code_office$Department, dept_zip_code_ref$`Department Name`)]

coordinates <- geocode_zip(vol_zip_code_office$dept_zip_code)
vol_zip_code_office <- merge(vol_zip_code_office, coordinates, by.x = c("dept_zip_code"), by.y = c("zipcode"), all.x = TRUE)
...
saveRDS(monthly_vol_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/monthly_vol_office.rds")
...
saveRDS(vol_zip_code_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/vol_zip_code_office.rds")
...
## Wait Time Horizon by Specialty
saveRDS(waitTime_dist_specialty_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/waitTime_dist_specialty_office.rds")
saveRDS(waitTime_dist_site_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/waitTime_dist_site_office.rds")

## Median Wait Time
saveRDS(waitTime_mshs_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/waitTime_mshs_office.rds") # By MSHS Specialty (Exc. MSDD and Network)
saveRDS(waitTime_site_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/waitTime_site_office.rds") # By Specialty and Site
saveRDS(waitTime_prov_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/waitTime_prov_office.rds") # By Specialty and Provider
...
saveRDS(payer_mix_quarter_site_office, "/nfs/data/Applications/Ambulatory/Quarterly_Data/payer_mix_quarter_site_office.rds")
