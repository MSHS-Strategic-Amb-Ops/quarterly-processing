...
```{r Turns Utilization by Location Summary Table, echo = FALSE, warning = FALSE, message = FALSE}
...
location_main_util_turns <- merge(location_main_volume, location_main_rooms,
                            by = c("Campus","Campus.Specialty"), all.x = TRUE)

location_sub_util_turns <- merge(location_sub_volume, location_sub_rooms,
                            by = c("Campus","Campus.Specialty"), all.x = TRUE)
...
```{r Durations Utilization by Location Summary Table, echo = FALSE, warning = FALSE, message = FALSE}
...
location_main_util_hour <- location_main_util_hour %>%
  ...
  filter(Appt.YearQtr == "2022 Q2") %>%
  filter(Appt.Day %in% c("Mon","Tue","Wed","Thu","Fri")) %>%
  dplyr::select(Campus, Campus.Specialty, Appt.YearQtr, Appt.DateYear, Appt.Day, 
                Visit.Method, Session, `08:00`:`19:00`) %>%
  pivot_longer(`08:00`:`19:00`,
               names_to = "space_time",
               values_to = "space_dur") %>%
  group_by(Campus, Campus.Specialty, Appt.YearQtr, Appt.DateYear, Appt.Day, 
           Visit.Method, Session, space_time) %>%
  summarise(total_dur = sum(space_dur)) %>%
  group_by(Campus, Campus.Specialty, Appt.YearQtr, Appt.Day, 
           Visit.Method, Session, space_time) %>%
  summarise(avg_dur = round(sum(total_dur)/length(unique(Appt.DateYear)))) 
...
# Utilization by Location: Rooms and Volume Data Merge -------------------------
location_main_util_hour <- merge(location_main_util_hour, location_main_rooms,
                            by = c("Campus","Campus.Specialty"), all.x = TRUE)

location_sub_util_hour <- merge(location_sub_util_hour, location_sub_rooms,
                            by = c("Campus","Campus.Specialty"), all.x = TRUE)
...
```{r Output Calculated Data Tables, echo = FALSE, warning = FALSE, message = FALSE}

# Utilization Data -------------------------------------------------------------
saveRDS(location_util_turns_merged, "locations_util_turns_merged.rds") # By Turns
saveRDS(location_util_dur_merged, "locations_util_dur_merged.rds") # By Turns
