...
scheduling_data_raw <- as.data.frame(readRDS("/nfs/data/Applications/Ambulatory/Data_Updated/historical_data.rds")) %>%
  # as.data.frame(readRDS(file.choose())) %>%
  filter(!is.na(Campus)) %>%
...
# Departments Managed by Department 
...
scheduling_data_raw$Managed <- master_amb_mapping$`Managed by Department or Site`[match(scheduling_data_raw$Department, master_amb_mapping$DEPARTMENT_NAME)]
scheduling_data_raw <- scheduling_data_raw %>%
  mutate(Managed = ifelse(is.na(Managed), "Department", Managed)) %>%
  filter(Managed == "Department")

# Departments Managed by Site
...
conn1 <- odbcConnect("SQLServer_Caboodle")

# Import Referrals Table
referrals <- sqlQuery(conn1, "SELECT * FROM PRDREPORT.FullAccess.Referrals")
...
scheduling_data_raw <- merge(scheduling_data_raw, epic_specialty_mapping, by = "Campus.Specialty") # Map Specialty Groups
...
referrals <- merge(referrals, site_dept_mapping, by.x = c("ReceivedByDept"), by.y = c("Department")) 
referrals <- merge(referrals, epic_specialty_mapping, by = "Campus.Specialty") # Map Specialty Groups

# Referrals Data - Departments Managed by Department
referrals$Managed <- master_amb_mapping$`Managed by Department or Site`[match(referrals$ReceivedByDept, master_amb_mapping$DEPARTMENT_NAME)]
referrals <- referrals %>%
  mutate(Managed = ifelse(is.na(Managed), "Department", Managed)) %>%
  filter(Managed == "Department")
...
monthly_referral_vol <- referrals_processed %>%
  group_by(Campus, Campus.Specialty.Group, Campus.Specialty.SubGroup, ReceivedBySite, ReceivedByDept, StartInstant, ApptStatusFinal) %>%
  summarise(total = n()) %>%
...
waitTime_dist_specialty <- scheduling_data_raw %>%
  ...
  filter(Campus %in% c("MSM","MSH-MSDFP","ONCOLOGY","MSW","MSBI","MSUS","MSH- AMBULATORY CARE")) %>%
...
# Referrals Tables -------------------------------------------------------------
saveRDS(monthly_referral_vol, "monthly_referral_vol.rds")
...
saveRDS(waitTime_dist_specialty, "waitTime_dist_specialty.rds")
saveRDS(waitTime_dist_site, "waitTime_dist_site.rds")

## Median Wait Time
saveRDS(waitTime_mshs, "waitTime_mshs.rds") # By MSHS Specialty (Exc. MSDD and Network)
saveRDS(waitTime_site, "waitTime_site.rds") # By Specialty and Site
saveRDS(waitTime_prov, "waitTime_prov.rds") # By Specialty and Provider

# Payor Mix Table --------------------------------------------------------------
saveRDS(payer_mix_quarter_site, "payer_mix_quarter_site.rds")
